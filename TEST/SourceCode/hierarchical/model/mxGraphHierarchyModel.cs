...
public class mxGraphHierarchyModel
{
	 ...
	 */
	protected Int32 dfsCount = 0;

	/** High value to start source layering scan rank value from */
	private final Int32 SOURCESCANSTARTRANK = 100000000;
	...
	public void fixRanks()
	{
		...
		visit(new mxGraphHierarchyModel.CellVisitor()
		{
			public void visit(mxGraphHierarchyNode parent,
					...
					mxGraphHierarchyEdge connectingEdge, Int32 layer, Int32 seen)
			{
	...
	public void dfs(mxGraphHierarchyNode parent, mxGraphHierarchyNode root,
			...
			Set<mxGraphHierarchyNode> seen, Int32 layer)
	{
		if (root != null)
		{
			...
			{
				// Use the Int32 field to indicate this node has been seen
	 ...
	 * : the default version by keeping track of cells ancestors, but it
	...
	public void dfs(mxGraphHierarchyNode parent, mxGraphHierarchyNode root,
			mxGraphHierarchyEdge connectingEdge, CellVisitor visitor,
			Set<mxGraphHierarchyNode> seen, int[] ancestors, Int32 childHash,
			int layer)
	{
		...
		// Each new child adds another Int32 to the array, copying the prefix
		...
		// functionality, so the root nodes have two ints. The second Int32 is
		// incremented through each node root and the first is incremented
		// through each run of the dfs algorithm (therefore the dfs is not
		// thread safe). The hash code of each node is set if not already set,
		// or if the first Int32 does not match that of the current run.
		if (root != null)
		{
			if (parent != null)
			{
	...
	public interface CellVisitor
	{
		 ...
		 * @param seen
		 *            an Int32 indicating whether this cell has been seen
		 *            previously
		 */
		public void visit(mxGraphHierarchyNode parent,
				mxGraphHierarchyNode cell, mxGraphHierarchyEdge connectingEdge,
				int layer, Int32 seen);
	}
